...
# Función para ajustar el scroll
...
def mostrar_graficas():
    ...
    # Crear las gráficas estáticas con Matplotlib (solo las primeras 5 gráficas)
    fig, axs = plt.subplots(5, 1, figsize=(12, 24), constrained_layout=True)  # Hacer las gráficas más grandes

    # Cambiar el color de fondo de las gráficas para que coincida con la interfaz
    fig.patch.set_facecolor('#2b2b2b')  # Asumimos que este es el color del fondo de la interfaz
    
    # Mapa de nivel
    axs[0].set_title('Mapa de Nivel', color='white')  # Color del título
    im = axs[0].imshow(elevation, cmap='terrain')
    fig.colorbar(im, ax=axs[0], label='Elevación (m)')
    axs[0].set_facecolor('#2b2b2b')  # Cambiar el color de fondo de la gráfica
    
    # Mapa de pendiente
    axs[1].set_title('Mapa de Pendiente', color='white')
    im = axs[1].imshow(slope_normalized, cmap='viridis')
    fig.colorbar(im, ax=axs[1], label='Pendiente Normalizada')
    axs[1].set_facecolor('#2b2b2b')
    
    # Mapa de zonas de acumulación
    axs[2].set_title('Zonas de Acumulación', color='white')
    im = axs[2].imshow(flow_accumulation, cmap='Blues')
    fig.colorbar(im, ax=axs[2], label='Acumulación de Flujo')
    axs[2].set_facecolor('#2b2b2b')
    
    # Mapa de superposición de pendiente y flujo
    axs[3].set_title('Superposición Pendiente/Flujo', color='white')
    axs[3].imshow(slope_normalized, cmap='viridis', alpha=0.7)
    im = axs[3].imshow(flow_normalized, cmap='hot', alpha=0.5)
    fig.colorbar(im, ax=axs[3], label='Pendiente Normalizada y Flujo')
    axs[3].set_facecolor('#2b2b2b')
    
    # Mapa de puntos de máximo flujo
    axs[4].set_title(f'Top {n} Puntos de Máximo Flujo', color='white')
    axs[4].imshow(elevation, cmap='terrain')
    for i, (coord_x, coord_y, magnitude_norm, structure) in enumerate(classified_points):
        ...
        axs[4].plot(x_plot, y_plot, 'ro')
    axs[4].set_facecolor('#2b2b2b')
    
    # Insertar las gráficas estáticas en la interfaz
    canvas_figura = FigureCanvasTkAgg(fig, master=frame_interior)
    canvas_figura.draw()
    canvas_figura.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=True)
...
# Pestaña 2: Mostrar Gráficas con Scroll
pestana_graficas = tabview.add("Mostrar Gráficas")

# Crear un Frame contenedor y un Canvas para manejar el scroll
frame_con_canvas = ctk.CTkFrame(pestana_graficas)
frame_con_canvas.pack(fill="both", expand=True)

canvas = tk.Canvas(frame_con_canvas)
canvas.pack(side="left", fill="both", expand=True)

scrollbar = tk.Scrollbar(frame_con_canvas, orient="vertical", command=canvas.yview)
scrollbar.pack(side="right", fill="y")

canvas.configure(yscrollcommand=scrollbar.set)

# Crear un Frame dentro del Canvas donde se colocarán las gráficas
